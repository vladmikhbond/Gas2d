...
type _Device = {x1: number, y1: number, x2: number, y2: number, type: string, rate: number, color: string, shift: number};
...
export default class Image 
{
    ...
    {    
        ...
        for (let d of this.space.devices()) {
            ...
            let device: _Device = {x1: d.x1, y1: d.y1, x2: d.x2, y2: d.y2, type, rate:0, color:'', shift:0};
            devices.push(device);
    ...
    {
        let {bombs, lines, plungers, devices, glo} = JSON.parse(json);
        ...
        devices.forEach((d: _Device) => {
            switch (d.type) {
                case 'm': 
                    space.addDevice(new Measurer(d.x1, d.y1, d.x2, d.y2, d.color, d.shift));
                    break;
                case 'h': 
                    space.addDevice(new Heater(d.x1, d.y1, d.x2, d.y2, d.rate));
                    break;
            } 
        });
